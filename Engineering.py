    ...
    def autosearch(self):
        for now in self.progression_iter():
            ...
            if now == 0:  # test traditional searcher
                Searcher("username", 0, 100, string).search()
                print("test usr complete")
            elif now == 1:  # test number searcher
                Searcher("username_NO", 0, 100, " ", ["2015", "2016"]).search()
                print("test usr_NO complete")
            elif now == 2:  # test chinese name searcher
                Searcher("username_CN", 0, 1).search()
                print("test usr_CN complete")
            elif now == 3:  # test id password searcher
                Searcher("password_ID", 0, 1000, list=["2016K8009909003"]).search()
                print("test usr_NO complete")
            elif now == 4:
